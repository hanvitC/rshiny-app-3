...
ui <- navbarPage(
  ...
  tabPanel("Exploratory Data Analysis",
           sidebarLayout(
             sidebarPanel(
               h4("EDA Options"),
               selectizeInput("eda_columns", "Select Columns for EDA", choices = NULL, multiple = TRUE, 
...
server <- function(input, output, session) {
  ...
  output$download_fe <- downloadHandler(
    filename = function() { "engineered_data.csv" },
    content = function(file) {
  ...
  output$eda_plot <- renderPlotly({
    ...
    if (input$plot_type == "Scatter Plot") {
      ...
      p <- plot_ly(df, x = ~df[[input$scatter_x]], y = ~df[[input$scatter_y]],
                   type = "scatter", mode = "markers", source = "scatterPlot")
      p <- p %>% layout(title = paste("Scatter Plot:", input$scatter_x, "vs", input$scatter_y),
      ...
      if (input$pca_mode == "3D" && input$pca_components >= 3) {
        if (!is.null(input$pca_group) && input$pca_group != "None") {
          p <- plot_ly(pca_data, x = ~PC1, y = ~PC2, z = ~PC3, color = ~pca_data[[input$pca_group]],
        ...
        p <- p %>% layout(title = "PCA Plot (First Two Components)",
      ...
      if (input$multi_cluster) {
        ...
        p <- plot_ly(type = "parcoords",
                     dimensions = lapply(names(sub_df)[-ncol(sub_df)], function(col) list(
                       label = col,
                       values = sub_df[[col]],
        ...
        km <- kmeans(sub_df, centers = 3)
        sub_df$cluster <- as.factor(km$cluster)
        p <- plot_ly(sub_df, x = ~sub_df[[input$cluster_x]], y = ~sub_df[[input$cluster_y]],
                     color = ~cluster, colors = "Set1", type = "scatter", mode = "markers", source = "clusterPlot")
        p <- p %>% layout(title = paste("Cluster Plot:", input$cluster_x, "vs", input$cluster_y),
      ...
      p <- plot_ly(df, x = ~df[[input$violin_group]], y = ~df[[input$violin_value]], type = "violin",
                   box = list(visible = TRUE), meanline = list(visible = TRUE),
                   name = paste(input$violin_value, "by", input$violin_group), showlegend = TRUE)
      p <- p %>% layout(title = paste("Violin Plot of", input$violin_value, "by", input$violin_group),
  ...
  output$download_stats <- downloadHandler(
    filename = function() { "eda_stats.csv" },
    content = function(file) {
      df <- cleaned_data()[, input$eda_columns, drop = FALSE]
      stat_list <- lapply(names(df), function(col) {
        if (is.numeric(df[[col]])) {
  ...
  output$download_pca <- downloadHandler(
    filename = function() { "pca_components.csv" },
    content = function(file) {
      req(input$eda_columns, input$pca_components)
      df <- cleaned_data()[, input$eda_columns, drop = FALSE]
      df <- df[, sapply(df, is.numeric), drop = FALSE]
  ...
  output$download_train <- downloadHandler(
    filename = function() { "train_set.csv" },
    content = function(file) {
  ...
  output$download_test <- downloadHandler(
    filename = function() { "test_set.csv" },
    content = function(file) {
  ...
  output$model_plot <- renderPlotly({
    ...
    if (is.numeric(cleaned_data()[[input$target_col]])) {
      ...
      p <- plot_ly(test, x = ~pred, y = ~test[[input$target_col]],
                   type = "scatter", mode = "markers", marker = list(color = "blue"))
      p <- p %>% layout(title = "Predicted vs Actual", xaxis = list(title = "Predicted"),
      ...
      p <- p %>% layout(title = "Histogram of Predicted Probabilities",
  ...
  output$download_model <- downloadHandler(
    filename = function() { "model_summary.txt" },
    content = function(file) {
      sink(file)
